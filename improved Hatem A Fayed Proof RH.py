...
# Euler-Maclaurin Formula with B2, B4, B6, B8, B10 Terms
def euler_maclaurin_zeta(s, N, include_B2=True, include_B4=True, include_B6=True, include_B8=True, include_B10=False):
    """
    Approximates zeta(s) using Euler-Maclaurin formula with optional B2, B4, B6, B8, B10 terms.
    s: complex number
    N: number of terms in partial sum
    include_B2, include_B4, include_B6, include_B8, include_B10: include respective Bernoulli terms
    """
...
# Perron’s Formula with Precomputed Zeta Values
def perron_formula(s, N, c, T, segments=8):
    """
    Computes partial sum zeta_N(s) using Perron's formula with precomputed zeta values.
    s: complex number
    N: number of terms
    ...
    z_values = [c + t * 1j for t in mp.linspace(-T, T, 1000)]
    zeta_values = [zeta(s + z, derivative=0) for z in z_values]
    def integrand(t):
        idx = int((t + T) / (2 * T) * (len(z_values) - 1))
        z = c + t * 1j
        return zeta_values[idx] * mp.power(N, z) * mp.power(2 * pi, -z) / z
    segment_size = 2 * T / segments
    result = mp.mpf(0)
    for i in range(segments):
        t_start = -T + i * segment_size
        t_end = t_start + segment_size
        result += quad(lambda t: integrand(t), [t_start, t_end], method='tanh-sinh')
...
for N1 in N1_values:
    ...
    approx_s1 = euler_maclaurin_zeta(s1, N1, include_B2=False, include_B4=False, include_B6=False, include_B8=False, include_B10=False)
    approx_s1_B2 = euler_maclaurin_zeta(s1, N1, include_B2=True, include_B4=False, include_B6=False, include_B8=False, include_B10=False)
    approx_s1_B4 = euler_maclaurin_zeta(s1, N1, include_B2=True, include_B4=True, include_B6=False, include_B8=False, include_B10=False)
    approx_s1_B6 = euler_maclaurin_zeta(s1, N1, include_B2=True, include_B4=True, include_B6=True, include_B8=False, include_B10=False)
    approx_s1_B8 = euler_maclaurin_zeta(s1, N1, include_B2=True, include_B4=True, include_B6=True, include_B8=True, include_B10=False)
    approx_s1_B10 = euler_maclaurin_zeta(s1, N1, include_B2=True, include_B4=True, include_B6=True, include_B8=True, include_B10=True)
    print(f"s = {s1}, N = {N1}:")
...
s2 = mp.mpf('0.6') + mp.mpf('10') * 1j
N2_values = [100, 1000, 10000, 100000, 1000000, 10000000]
true_zeta_s2 = zeta(s2)
print(f"\ns = {s2}:")
print(f"True zeta(s): {true_zeta_s2}")
for N2 in N2_values:
    approx_s2 = euler_maclaurin_zeta(s2, N2, include_B2=True, include_B4=False, include_B6=False, include_B8=False, include_B10=False)
...
c = mp.mpf('1.1')
T = mp.mpf('1000')
perron_result = perron_formula(s_perron, N_perron, c, T, segments=16)
actual_partial_sum = sum(mp.power(n, -s_perron) for n in range(1, N_perron+1))
print(f"s = {s_perron}, N = {N_perron}, c = {c}, T = {T}, segments = 16:")
